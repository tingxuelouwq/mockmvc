...
// 注意：必须要使用@RunWith指定Theories.class
...
public class TheoryTest {
    ...
    // 和name是不起任何作用的，"2007"同样有机会传给参数name，"Works"也同样有机会传给参数year
    @DataPoint
    ...
    public void testNames2( String year, String name ) {
        assumeThat(year, is("2007")); //year必须是"2007"，否则跳过该测试函数
        assumeThat(name, allOf( not(is("2007")), not(is("2008")))); //name必须既不是"2007"也不是"2008"，否则跳过该测试函数
        System.out.println( year + "-" + name );
        assertThat( year, is("2007") ); //这里的断言语句没有实际意义，这里举此例只是为了不中断测试
    }
}
